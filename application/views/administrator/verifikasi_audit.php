...
<section class="panel">
    <header class="panel-heading">
        <h2 class="panel-title">Verifikasi Audit</h2>
    </header>
        ...
        <table class="table table-bordered table-striped" id="datatable-ajax">
            ...
            <?php $n = 1; foreach ($tampil as $u){ ?>
                <tr>
                    ...
                    <td>
                        <?php
                        if($u->status_validasi == 0) {
                            ?>
                            <a href="<?=base_url('admin/updateStatus/'.$u->id_perusahaan.'/1')?>" class="btn btn-xs btn-success"><li class="fa fa-check"></li></a>
                            <a href="<?=base_url('admin/updateStatus/'.$u->id_perusahaan.'/2')?>"class="btn btn-xs btn-danger"><li class="fa fa-times"></li></a>
                        <?php }
                        else if($u->status_validasi == 1) {
                            ?>
                            <button class="btn btn-xs btn-success"><li class="fa fa-check"></li></button>
                        <?php }
                        else {
                            ?>
                            <button class="btn btn-xs btn-danger"><li class="fa fa-times"></li></button>
                            <?php
                        }
                        ?>
                    </td>
